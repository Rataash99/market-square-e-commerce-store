                ...
                <div className='w-full h-[72.8vh] p-7  grid grid-cols-2 gap-6 text-xs overflow-scroll sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 sm:text-base  sm:hover:border-y sm:hover:border-cyan-300 sm:hover:border-opacity-25'>
                {
                    products && products.map((item, index) => (
                        <Link to={`/products/${item.id}`} key={index}>
                            <div className="card flex flex-col justify-between rounded-lg p-3 shadow-md shadow-cyan-900 hover:shadow-l bg-gradient-to-bl from-black via-slate-900 to-black duration-500 hover:scale-105 ease-out h-[18rem] sm:h-[28rem] lg:h-[32rem] hover:shadow-lg hover:shadow-cyan-900">
                                ...
                                <div className='flex gap-4 flex-col overflow-scroll'>
                                    <p className='text-[0.6rem] sm:text-sm text-sky-400'>{item.title}</p>
