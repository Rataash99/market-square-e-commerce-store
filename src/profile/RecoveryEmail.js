...
const RecoveryEmail = (props) => {
    ...
    const {recoveryEm, setRecoveryEm} = props.recoveryEmail
  ...
  return (
    ...
    <section className='bg-gradient-to-tr from-black via-slate-900 to-black flex gap-2 md:gap-3 lg:gap-4 flex-col h-auto pb-10 rounded-md shadow-md shadow-sky-800 p-2 w-[80vw] sm:max-w-lg'>
        <div className='flex  justify-between p-2 gap-10'>
            ...
            <h1 className='text-base sm:text-lg md:text-xl lg:text-2xl tracking-wide font-semibold bg-clip-text text-transparent bg-gradient-to-r from-sky-500 to-indigo-400 text-center'>Recovery Email</h1>
            <img className='w-7 sm:w-8 md:w-9 my-auto lg:w-10 animate-wiggle cursor-pointer' src={close} onClick={() => setRecoveryEm(false)} />
        </div>
        <form className='space-y-3 md:space-y-4 p-3 rounded-sm border-slate-800 border' onSubmit={handleSubmit}>
            ...
            <label className='relative'>
                    <input className='rounded-sm bg-transparent border p-2 border-slate-800 w-full' type= 'email' placeholder='Phone Number' name='pincode' required onChange={(e) => addDetails(e)}/>
                </label>
            </div>
