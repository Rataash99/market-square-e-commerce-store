    ...
    return (
        <section className='rounded-md overflow-scroll sm:mx-auto'>
            <div className=' gap-2 flex flex-col '>
                {
                    !length > 0 ? <div className='my-auto border'>
                        ...
                        : globalData.cartArr.map((item, index) => (
                            <div className='space-y-5 bg-gray-800 rounded-md p-3'>
                                ...
                                <div>
                                    <Link to={`/products/${item.id}`} className=' p-1 rounded-md text-cyan-500 hover:text-cyan-400 px-3 text-center text-xs bg-gradient-to-br from-black via-slate-900 to-black'>View order</Link>
                                </div>
                            </div>
                        ))
                }
